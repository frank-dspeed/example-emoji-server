...
// String.fromCodePoint(...codesAsCodePoints('1F1FF 1F1FC'))
export const codeToCodePoint = code=>`0x${code}`;
export const codePointsToCode = codePoints => codePoints.map(x=>x.split('0x')[1]).join(' ')
const faces = data
    .split('\n')
    .map(line => { 
    ...
    .map(line=> {
        ...
        const Codes = csv[0].split(' ');
        const emojiSourcesAndComment = csv[4];
        
        if (emojiSourcesAndComment && emojiSourcesAndComment.indexOf('(') > -1) {
            const codePoints = Codes.map(codeToCodePoint);
            const desc = line.split(')')[1].trim()
            // Only return Faces
            if (desc.indexOf('FACE') > -1) {
                return { Codes: csv[0], codePoints, desc }
            }
        }
        return undefined;
    })
